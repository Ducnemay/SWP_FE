...
const NavBar = () => {
  ...
  const searchArtistByName = async (name) => {
    try {
      const response = await api.get(`https://localhost:7227/api/User/search-by-name?user=${name}`);
      // Trả về danh sách các artwork từ response API hoặc một mảng rỗng nếu không có kết quả
      return response.data.$values || [];
    } catch (error) {
      throw new Error(error.response.data);
  ...
  const handleSearch = async (searchType) => {
    try {
      ...
      if (searchType === 'artwork') {
        ...
        searchResults = await searchArtistByName(searchInput);
      }
  ...
  {searchInput.trim() !== '' && (
        <div className="search-results">
          {searchResults && searchResults.length > 0 ? (
            searchResults.map((result, index) => (
              <div key={index} className="search-result-item">
                <div className="image-wrapper" onClick={() => navigate(`/artist/${result.userId}`)}>
                  <img src={result.imageUrl} alt={result.title} />
                </div>
                <div className="result-details">
      ...
      <div className="nav-list">
        <ul>
          <li><NavLink to="/artworks" className="Nav_link" style={{  textDecoration:"none"}}>Artworks</NavLink></li>
          <li><NavLink to="/auction"className="Nav_link" style={{  textDecoration:"none"}}>Auction</NavLink></li>
          <li><NavLink to="/viewing-room"className="Nav_link" style={{  textDecoration:"none"}}>Viewing Room</NavLink></li>
          <li><NavLink to="/galleries"className="Nav_link" style={{  textDecoration:"none"}}>Galleries</NavLink></li>
          <li><NavLink to="/fairs-shows"className="Nav_link" style={{  textDecoration:"none"}}>Fairs Shows</NavLink></li>
